...
- Das Debian-Paket `reprapfirmware` (Abhängigkeit von `duetsoftwareframework`) liefert die Duet-Board-Firmware bereits nach `/opt/dsf/sd/firmware`; `duetwebcontrol` liefert DWC nach `/opt/dsf/dwc`. Der GitHub-Download aus DuetPi entfällt damit. `AutoUpdateFirmware: true` in `config.json` flasht das Board bei Versionsabweichung, also auch nach OTA/Rollback.
...
- **Identity-Datei** `/bootfs/meltingplot/identity.conf` (BOOTCONFIG-Partition, FAT, rw, vom PC beschreibbar; Vorlage `identity.conf.example`):
  ```
  printer_serial=0042            # Meltingplot-Seriennummer des CHX350 (Pflicht)
  printer_name=Halle 2 links     # optional: Anzeigename in DWC/PanelDue (M550); Default "Meltingplot CHX-350 0042"
...
- `mp-identity.service` (Type=oneshot, DefaultDependencies=no, `RequiresMountsFor=/bootfs /persistent`, After=`persistent.mount machine-id-sync.service`, Before=`mp-hostname.service`), Skript `/usr/sbin/mp-identity`:
  1. Datei vorhanden → `printer_serial` validieren (`^[A-Za-z0-9-]{1,32}$`, lowercase) und nach `/persistent/common/etc/mp-identity` (`PRINTER_SERIAL=…`, `ROLE=sbc`, optional `PRINTER_NAME=…`, druckbare Zeichen ohne `"`) sowie Hostname `meltingplot-chx-350-<serial>-sbc` nach `/persistent/common/etc/hostname` schreiben (nur wenn dort noch nichts steht; eine Neuzuordnung erfordert `force=1` in der Datei). `PRINTER_NAME` wird von `mp-dsf-seed` beim Anlegen von `printer-name.g` verwendet, sonst der Default mit Seriennummer.
...
- Konsequenzen (dokumentieren): Firmware-Upload per DWC (EROFS) und M997-Selbstupdate funktionieren nicht; Updates kommen als OTA-Bundle. Nachträglich installierte SBC-Plugins werden nie ausgeführt, DWC-Plugins und das mitgelieferte Vigil funktionieren. DWC-Settings, Heightmaps etc. liegen unter `sd/sys` → persistent.
...
3. Gerät: `rpi-connectd` lädt das Bundle, der Root-Agent aus `rpi-connect-ota` schreibt `boot`/`system` in den inaktiven Slot (`/dev/disk/by-slot/other/*`), tryboot, Health-Check, Commit (`/bootfs/autoboot.txt`) oder Rückfall auf den alten Slot. Die konkreten Health-/Commit-Kriterien sind nicht dokumentiert und nicht quelloffen → im Labor per `dpkg -L rpi-connect-ota` und Journal ermitteln, bevor der erste Kundenrollout erfolgt.
4. Voraussetzung auf dem Gerät: `RPI_CONNECT_EXPERIMENTAL_OTA=t` (Layer) bzw. `rpi-connect ota on`; Zustände im Dashboard: Pending → In Progress → Succeeded/Failed; Logs `journalctl -t rpi-ota-connector`.
5. Timing für Drucker: Ein Update darf nie während eines Drucks umbooten. Da das Deployment sofort startet, gilt organisatorisch „nur bei stehender Maschine deployen“; technisch Folgeaufgabe: Inhibit-Mechanismus (systemd-inhibit/`DCS`-Status `printing` → Reboot verzögern), sobald die Hook-Punkte von rpi-connect-ota bekannt sind.

### Rollback
- Automatisch: Bootloader-Tryboot fällt bei Boot-Fehler auf `[all] boot_partition` zurück; rpi-connect-ota rollt bei fehlgeschlagenem Health-Check zurück (zu verifizieren).
- Manuell per Connect: vorheriges Artefakt erneut deployen (Artefakte bleiben registriert).
- Manuell per Shell: `rpi-slot-tryboot > /bootfs/autoboot.txt` und `reboot '0 tryboot'` (Wechsel in den anderen Slot; Commit durch erneutes Schreiben nach erfolgreichem Boot). `/persistent` bleibt bei beidem unangetastet; Konfigurationsdateien werden beim Boot vom dann aktiven Slot gepusht (ältere DWC/Config zurück), geschützte Dateien bleiben.
...
8. Hardware: Pi5, SD ≥ 16 GB (Connect-OTA-Mindestgröße), Duet-3-Board am SBC-Ribbon. Board-Firmware wird von DSF automatisch auf 3.7.0-rc.1 gebracht; `chx350-config` ist auf RRF 3.6.1 geschrieben – Fehler in `config.g` unter 3.7 sind möglich, blockieren den Durchstich aber nicht.
...
- `systemctl status duetcontrolserver duetwebserver`; `journalctl -u duetcontrolserver` zeigt „Connection to Duet established“; DWC über `http://10.42.0.2/` und `http://<hostname>.local/`; `M115` liefert Board-Version; ggf. Auto-Firmware-Update beobachten.
...
- rpi-connect-ota: Health-Check-, Commit- und Timeout-Logik sowie Verhalten bei laufendem Druck sind undokumentiert; vor dem ersten Kundenrollout im Labor ermitteln. Deployments nur pro Gerät und ohne API → Rollout-Aufwand skaliert linear mit der Flotte; Management-API auf Deployment-Endpunkte beobachten.
